  ...
  module RequestSources
    ...
    class GitHub < RequestSource
      ...
      def ensure_danger_branches_are_setup
        ...
        test_branch = self.dsl.base_commit

        # Next, we want to ensure that we have a version of the current branch at a known location
        self.scm.exec "branch #{self.environment.danger_base_branch} #{test_branch}"

        # OK, so we want to ensure that we have a known head branch, this will always represent
        # the head of the PR ( e.g. the most recent commit that will be merged. )
        self.scm.exec "fetch origin +refs/pull/#{pull_id}/merge:#{danger_head_branch}"
      end
